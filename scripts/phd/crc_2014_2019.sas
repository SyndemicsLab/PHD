...
PROC SQL;
    CREATE TABLE merged_obs_&TWOYR. (DROP=rawID merge1ID merge2ID merge3ID) AS
        SELECT COALESCE(merge3.merge3ID, matris.rawID) AS ID, * FROM (SELECT
        COALESCE(merge2.merge2ID, pmp.rawID) AS merge3ID, * FROM (SELECT
        COALESCE(merge1.merge1ID, death.rawID) AS merge2ID, * FROM (SELECT
        COALESCE(apcd_cm_birth.rawID, bsas.rawID) AS merge1ID, * FROM
        apcd_cm_birth_&TWOYR. (RENAME=(ID=rawID)) AS apcd_cm_birth FULL JOIN
        bsas_&TWOYR. (RENAME=(ID=rawID)) AS bsas ON apcd_cm_birth.rawID=
        bsas.rawID) AS merge1 FULL JOIN death_&TWOYR. (RENAME=(ID=rawID)) AS
        death ON merge1.merge1ID=death.rawID) AS merge2 FULL JOIN pmp_&TWOYR.
        (RENAME=(ID=rawID)) AS pmp ON merge2.merge2ID=pmp.rawID) AS merge3 FULL
        JOIN matris_&TWOYR. (RENAME=(ID=rawID)) AS matris ON merge3.merge3ID=
        matris.rawID;
QUIT;
...
DATA merged_obs_&TWOYR._1;
    SET merged_obs_&TWOYR.;
    IF SUM(of oud_apcd_cm_birth oud_bsas oud_death oud_matris oud_pmp )=0 THEN
...
PROC SQL;
    CREATE TABLE merged_obs_&TWOYR._1 AS SELECT DISTINCT ID, oud_apcd_cm_birth,
        oud_bsas, oud_death, oud_matris, oud_pmp, sex_combo, race_combo FROM
        merged_obs_&TWOYR._1 WHERE race_combo IN (1,2,3,4,5) AND sex_combo IN
        (1,2) ;
QUIT;
...
PROC SUMMARY DATA=merged_obs_&TWOYR._1 NWAY COMPLETETYPES;
    CLASS oud_apcd_cm_birth oud_bsas oud_death oud_matris oud_pmp sex_combo
        race_combo /MISSING;
...
PROC SQL;
    CREATE TABLE Contingency_&TWOYR._noS AS SELECT oud_apcd_cm_birth, oud_bsas,
        oud_death, oud_matris, oud_pmp, SUM(raw_n) AS count FROM
...
PROC SQL;
    SELECT oud_apcd_cm_birth, oud_bsas, oud_death, oud_matris, oud_pmp,
        sex_combo, race_combo, count FROM Contingency_&TWOYR._2 ;
QUIT;
title2;
...
PROC HPGENSELECT DATA=Contingency_&TWOYR._noS;
    ...
    MODEL count=oud_apcd_cm_birth oud_bsas oud_death oud_matris oud_pmp /DIST=
        Poisson INCLUDE=(oud_apcd_cm_birth oud_bsas oud_death oud_matris
        oud_pmp) link=Log CL;
RUN;
ODS OUTPUT CLEAR;
ODS SELECT ALL;
...
PROC HPGENSELECT DATA=Contingency_&TWOYR._noS;
    CLASS oud_apcd_cm_birth(ref="0") oud_bsas(ref="0") oud_death(ref="0")
        oud_matris(ref="0") oud_pmp(ref="0") /param=ref;
    MODEL count=oud_apcd_cm_birth oud_bsas oud_death oud_matris oud_pmp /DIST=NB
        INCLUDE=(oud_apcd_cm_birth oud_bsas oud_death oud_matris oud_pmp) link=
        Log CL;
RUN;
ODS OUTPUT CLEAR;
ODS SELECT ALL;
        ...
        %DO j=1 %TO 4;
            ...
            PROC HPGENSELECT DATA=SEX&i._RACE&j._&TWOYR.;
                CLASS oud_apcd_cm_birth(ref="0") oud_bsas(ref="0")
                    ...
                    /param=ref;
                MODEL raw_n=
                    oud_apcd_cm_birth|oud_bsas|oud_death|oud_matris|oud_pmp@2/DIST
                    =Poisson INCLUDE=(oud_apcd_cm_birth oud_bsas oud_death
                    oud_matris oud_pmp) link=Log CL;
