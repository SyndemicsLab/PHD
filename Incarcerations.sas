...
PROC STDIZE DATA = oud_monthly OUT = oud reponly missing = 9999; RUN;
PROC STDIZE DATA = oud_yearly OUT = oud reponly missing = 9999; RUN;

DATA oud_monthly (KEEP= ID age_grp FINAL_RE FINAL_SEX year month);
    SET oud_monthly;
...
DATA oud_yearly (KEEP= ID age_grp_five age_grp_ten FINAL_RE FINAL_SEX year month);
    SET oud_yearly;
...
DATA doc;
	SET PHDDOC.DOC(KEEP= ID ADMISSION_YEAR_DOC RELEASE_YEAR_DOC
						 ADMISSION_MONTH_DOC RELEASE_MONTH_DOC);
	doc_status = 1;
RUN;

DATA doc_expanded_monthly;
	SET doc;
	DO year = ADMISSION_YEAR_DOC to RELEASE_YEAR_DOC;
		IF year = ADMISSION_YEAR_DOC THEN
		start_month = ADMISSION_MONTH_DOC;
		ELSE
		start_month = 1;

		IF year = RELEASE_YEAR_DOC THEN
		end_month = RELEASE_MONTH_DOC;
		ELSE
		end_month = 12;
	...
	drop ADMISSION_YEAR_DOC ADMISSION_MONTH_DOC RELEASE_MONTH_DOC RELEASE_YEAR_DOC;
RUN;

DATA doc_expanded_yearly (keep= ID year doc_status);
	SET DOC;
	DO year = ADMISSION_YEAR_DOC to RELEASE_YEAR_DOC;
		output;
	END;
...
PROC SQL;
	CREATE TABLE doc_merged_yearly AS
	SELECT DISTINCT * FROM oud_yearly
	LEFT JOIN (
		SELECT DISTINCT year, doc_status, ID FROM doc_expanded_yearly
	) as doc_yearly
	ON doc_yearly.ID = oud_yearly.ID AND
		doc_yearly.year = oud_yearly.year;

	CREATE TABLE doc_merged_monthly AS
	SELECT DISTINCT * FROM oud_monthly
	LEFT JOIN (
		SELECT DISTINCT year, month, doc_status, ID FROM doc_expanded_monthly
	) AS doc_monthly
	ON doc_monthly.ID = oud_monthly.ID AND
		doc_monthly.year = oud_monthly.year AND
		doc_monthly.month = oud_monthly.month;
QUIT;

DATA incar_yearly;
	SET doc_merged_yearly;
	BY ID;
    retain oud_extended;

    IF first.ID THEN oud_extended = 0;
    IF doc_status = 1 and lag(doc_status) ne 1 THEN oud_extended = 0;
    ELSE IF oud = 1 THEN oud_extended = 1;
    oud = coalesce(oud_extended, oud);
	IF doc_status = . THEN doc_status = 0;
    drop oud_extended;
RUN;

DATA incar_monthly;
	SET doc_merged_monthly;
	BY ID;
    retain oud_extended;
...
PROC SQL;
	CREATE TABLE incar_yearly_ten AS
...
PROC EXPORT
	DATA= incar_yearly_ten
	OUTFILE= "/sas/data/DPH/OPH/PHD/FOLDERS/SUBSTANCE_USE_CODE/RESPOND/RESPOND UPDATE/Incarcerations_Ten_&formatted_date..csv"
	DBMS= csv REPLACE;
RUN;

PROC EXPORT
	DATA= incar_yearly_five
	OUTFILE= "/sas/data/DPH/OPH/PHD/FOLDERS/SUBSTANCE_USE_CODE/RESPOND/RESPOND UPDATE/Incarcerations_Five_&formatted_date..csv"
	DBMS= csv REPLACE;
RUN;
